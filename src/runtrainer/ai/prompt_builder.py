...
- 双乳酸阈值日（挪威模式；课表中 T1=上午段、T=下午段）：一天两练——上午段 LT1 巡航阈（≈84% VDOT、比 T 慢 5–10 秒/公里、体感「稳定而克制」、能说短句）立「量」，下午段 LT2 乳酸阈（≈88% VDOT、与文献群体 VT2 速度均值 87.6% 同档、「舒适地费力」、只能蹦单词）磨「强度」。操作义：T1 是「低于 LT2 的阈下巡航带」，不是 VT1/有氧阈（文献 VT1 速度群体均值仅 ≈74%，别混用术语）。双阈值日形态由菜单轮换（上午 5–6 分钟×多组或长段巡航、下午分钟巡航或 400/800/1000m 重复），按跑力已缩量（主段各 ≤35 分钟），不要再写「4×8′+5×5′」之类固定模板。实践规则：两练间隔 ≥6 小时并补碳水补水；业余先每周 1 个双阈值日、4–6 周稳定后再上 2 个；双阈值日前后一天安排真正轻松的课；心率高/恢复差时降为单段或 M/E；女性与 50 岁以上取每周 1 天的保守档。证据边界：双阈值 vs 经典训练无 RCT 头对头，证据是精英案例与急性拆场实验——措辞不要说「已被科学证实更优」。
...
- 双阈值/挪威模式：上午 LT1 巡航阈（比 LT2 慢 5–10 秒/公里）+ 下午 LT2 的当天两练，间隔 ≥6 小时；有精英案例与急性拆场实验支持，无 RCT 头对头——说明时给证据边界，别称「科学证实最优」。
