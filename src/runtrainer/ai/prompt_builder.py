...
ZONE_NAMES = {"E": "轻松跑", "M": "马拉松配速", "T1": "有氧阈 LT1", "T": "乳酸阈 LT2",
              "I": "间歇", "R": "重复跑"}
...
5. 配速只能用给定配速表（E/M/T1/T/I/R，T1=LT1 有氧阈、T=LT2 乳酸阈），不许编造其他配速区间。
...
- 双乳酸阈值日（挪威模式）：双阈值日是一天两练——上午段 LT1 有氧阈（≈2 mmol、约 84% VDOT、心率 75–80% HRmax，如 4×8′，比 T 慢 5–10 秒/公里）立有氧阈的「量」，下午段 LT2 乳酸阈（T 配速 ≈4 mmol、心率 85–90% HRmax，如 5×5′）磨乳酸阈的「强度」；两练间隔 ≥5 小时并补碳水补水，单段 ≤30 分钟、全天阈值总量 ≤60 分钟。课表里 T 即 LT2，T1 即 LT1 有氧阈（配速见配速表）。心率高/恢复差时提示改为单段或降为 M/E。
...
5. 配速只能用给定配速表（E/M/T1/T/I/R，T1=LT1 有氧阈、T=LT2 乳酸阈），不许编造配速区间；数据缺失时保守。
...
- 配速只能用给定配速表（E/M/T1/T/I/R，T1=LT1 有氧阈、T=LT2 乳酸阈），不许编造配速区间；数据缺失时保守；伤病指征优先建议休息。
